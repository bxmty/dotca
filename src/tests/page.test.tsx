...
describe('Boximity Landing Page', () => {
  ...
  it('renders essential landing page components', () => {
    ...
    const pricingButton = screen.getByRole('link', {
      name: /See Pricing Options/i,
    });
    expect(pricingButton).toBeInTheDocument();

    const assessmentButton = screen.getByRole('button', {
      name: /Get Your IT Assessment/i,
    });
    expect(assessmentButton).toBeInTheDocument();

    // Check for footer
    expect(
      screen.getByText('© 2025 boximity msp. All rights reserved.')
    ).toBeInTheDocument();
  });
  ...
  it('has pricing link with correct href', () => {
    ...
    const navPricingLink = pricingLinks.find(
