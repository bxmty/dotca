...
describe('OnboardingPage Component', () => {
  ...
  it('navigates to step 3 and back', async () => {
    ...
    expect(screen.getByLabelText(/Current IT Providers\/Services/i)).toBeInTheDocument();
    expect(screen.getByLabelText(/Key Software\/Applications Used/i)).toBeInTheDocument();
    expect(screen.getByLabelText(/Current IT Pain Points/i)).toBeInTheDocument();
    expect(screen.getByLabelText(/IT Goals for Next 12 Months/i)).toBeInTheDocument();
  ...
  it('updates form data when fields are changed', async () => {
    ...
    // Fill in step 3 form
    await user.type(screen.getByLabelText(/Current IT Providers\/Services/i), 'Current provider');
    await user.type(screen.getByLabelText(/Key Software\/Applications Used/i), 'Software 1, Software 2');
  ...
  it('submits form data and redirects on success', async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith('/api/onboarding', expect.any(Object));
